...
class ArticleController extends CommentController
{
	public function index(Request $request)
	{
		...
		if (!empty($request->all())) {
			...
			$article = json_decode($this->redis->get("article"));
			if (empty($article)) {
				$article = $array = DB::table("article")->join("label_gl","article.arid","=","label_gl.lgarid")->join("label","label_gl.lglaid","=","label.laid")->where(["arstatus"=>1])->orderBy("arid","desc")->select("article.arid","article.artitle","article.arftitle","article.arcreatetime","article.arcomcount","article.arbrowse","article.arauthod","label.laname")->get();
				$this->redis->set("article",$article);
			}
		}
